...
struct cmp {
    bool operator()(const pair<int, vector<vector<int>>>& lhs, const pair<int, vector<vector<int>>>& rhs) const {
        if(userChoice == 2) {
            return misplacedTile(lhs.second) > manhattanD(rhs.second);
        } else {
            return manhattanD(lhs.second) > manhattanD(rhs.second);
        }
    }
};
...
void searchHeuristic() {
  ...
  if(userChoice == 2) {
      pq.push(make_pair(misplacedTile(puzzle), puzzle));
  } else if(userChoice == 3) {
      pq.push(make_pair(manhattanD(puzzle), puzzle));
  }
  ...
  while(!pq.empty()) {
    ...
    for(int i = 0; i < toEnqueue.size(); i++) {
      if(!checkDuplicate(toEnqueue.at(i))) {
        if(userChoice == 2) {
          pq.emplace(make_pair(misplacedTile(toEnqueue.at(i)), toEnqueue.at(i)));
        } else if(userChoice == 3) {
          pq.emplace(make_pair(manhattanD(toEnqueue.at(i)), toEnqueue.at(i)));
        }
      }
    }
    depth++;
